export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      crsBack: '返回',
      // 分销站「从生产同步」
      syncFromProdTitle: '从生产同步账号',
      syncFromProdDesc: '只读连接生产主站，把「生产有、本站没有」的账号（含分组、代理）只新增到本站；已同步过的会自动跳过。',
      prodUpdateBehaviorNote: '只新增、不覆盖：已存在的账号（按同步来源 ID 匹配）会跳过，不会改动你手动设置的倍率；新建分组按默认加价系数创建，可事后手动调整。',
      prodPreviewing: '正在读取生产账号…',
      prodPreviewFailed: '读取生产账号失败',
      prodCandidates: '可同步账号（可选择）',
      prodNoCandidates: '没有可同步的新账号，生产账号均已同步。',
      prodSyncResultSummary: '新建账号 {created}，新建分组 {groups}，新建代理 {proxies}，跳过 {skipped}，失败 {failed}',
      prodSyncCompleted: '同步完成：新建账号 {created}，新建分组 {groups}，新建代理 {proxies}，跳过 {skipped}',
      prodSyncCompletedWithErrors: '同步完成但有错误：失败 {failed}（新建账号 {created}，跳过 {skipped}）',
      sourceSynced: '主站同步',
      sourceManual: '手动添加',
      editAccount: '编辑账号',
      deleteAccount: '删除账号',

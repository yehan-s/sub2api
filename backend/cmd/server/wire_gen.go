...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	resellerSyncConfig := service.ProvideResellerSyncConfig(configConfig)
	accountSyncService := service.NewAccountSyncService(accountRepository, proxyRepository, groupRepository, settingService, resellerSyncConfig)
	accountHandler := admin.NewAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, accountSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator)
	...
	ssoHandler := handler.NewSSOHandler(apiKeyService, userService, redisClient, configConfig)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, ssoHandler, idempotencyCoordinator, idempotencyCleanupService)
